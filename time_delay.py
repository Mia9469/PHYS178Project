...
## Functions 
"max(H1[index],H2[index])<=15"

def experiment():
	H1, H2, S1, S2 = np.zeros(steps+1), np.zeros(steps +
	                          1), np.zeros(steps+1), np.zeros(steps+1)
	Inoise1, Inoise2 = np.zeros(steps+1), np.zeros(steps+1)
	for index, t in enumerate(time):
		Inoise1[index+1] = Inoise1[index] + dt * \
		    (-Inoise1[index] + np.random.normal(0, 1, 1)
		     [0]*np.sqrt(tauampa*stdnoise**2))/tauampa
		Inoise2[index+1] = Inoise2[index] + dt * \
		    (-Inoise2[index] + np.random.normal(0, 1, 1)
		     [0]*np.sqrt(tauampa*stdnoise**2))/tauampa
		if (t > 0) & (t<2):
			x1 = Jn11*S1[index] - Jn12*S2[index] + I0 + \
			    Jext*mu0*(1+cprime/100) + Inoise1[index]
			x2 = Jn11*S2[index] - Jn12*S1[index] + I0 + \
			    Jext*mu0*(1-cprime/100) + Inoise2[index]
		else:
...
s1,s2,r1,r2 = quasi_dist(500,34000)

plt.figure
h=plt.hist2d(x=s1, y=s2, bins=200,cmin=1)
plt.xlabel('S2')
plt.ylabel('S1')
plt.show()

plt.figure
h_r=plt.hist2d(x=r1, y=r2, bins=100,cmin=1)
plt.xlabel('r2')
plt.ylabel('r1')
plt.show()
...
sio.savemat('trial2.mat', mdict={'r1':result[0],'r2':result[1],'S1':result[2],'S2':result[3]})
